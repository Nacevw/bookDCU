...
def upcoming_bookings(request):
    user = request.user
    # bookings = Booking.objects.filter(user_id=user, start_date__gte=date.now.strftime('%Y-%m-%d %H:%M'))
    futureBookings = []
    bookings = Booking.objects.filter(user_id=user)
    current_datetime = datetime.combine(date.today(), datetime.now().time())
    ...
    # create a variable bookings to store bookings that occur after today
...
def add_to_basket(request, roomid, seatnumber, date, start_time, end_time):
    ...
    if sbi is None:
        ...
        sbi.save()
        # return render(request, 'seat_selection.html', {'room': room, 'added': True})
...
def show_basket(request):
    # get the user object
    # does a shoping basket exist ? -> your basket is empty
...
def booking(request):
    ...
    if created:
        ...
        seat.save()
    # else:
    #     # if the seat already existed, check if it's already booked
    #     if seat.room_id.booked_seats.filter(seat_number=seat_number).exists():
    #         sbi.delete()
    #         # return a redirect to the booking-unavailable page
    #         return render(request, "booking-unavailable.html")
    #     else:
    #         seat.occupant_first_name = user.first_name
    #         seat.occupant_last_name = user.last_name
    #         seat.occupant_email = user.email
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and form.cleaned_data['name'] == user.first_name + " " + user.last_name:
            ...
            # else:
            #     # if the seat already existed, check if it's already booked
            #     if seat.room_id.booked_seats.filter(seat_number=seat_number).exists() and booking:
            #         sbi.delete()
            #         # return a redirect to the booking-unavailable page
            #         return render(request, "booking-unavailable.html")
            #     else:
